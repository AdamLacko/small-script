 ...
 *
 *  Version: $Id: Sc2.c 2419 2003-11-10 16:58:27Z thiadmer $
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
static int substpattern(char *line,size_t buffersize,char *pattern,char *substitution)
{
  ...
  for (prefixlen=0,start=pattern; *start!='\0' && *start!='%'; prefixlen++,start++)
    /* nothing */;
  assert(prefixlen>0);
  ...
  while (*start!='\0') {
    ...
    if (*start=='\0')
      break;            /* abort loop on error */
    /* do a quick compare on the first character to avoid the overhead of
     * a function call in the typical case
     */
    if (*start!=*pattern || strncmp(start,pattern,prefixlen)!=0) {
      while ((alpha(*start) || isdigit(*start)) && *start!='\0')
        start++;        /* skip this symbol */
      continue;         /* pattern prefix does not match */
    } /* if */
    ...
    if (match) {
      ...
      if (strlen(line) + len - (int)(s-start) > buffersize) {
        error(75);      /* line too long */
      } else {
        /* substitute pattern */
        strdel(start,(int)(s-start));
        for (e=substitution,s=start; *e!='\0'; e++) {
          if (*e=='%' && isdigit(*(e+1))) {
            arg=*(e+1)-'0';
            assert(arg>=0 && arg<=9);
            if (args[arg]!=NULL) {
...
static void substallpatterns(char *line,int buffersize)
{
  ...
  index=0;
  while (get_subst(index,&pattern,&substitution)) {
    if (substpattern(line,buffersize,pattern,substitution))
      index=0;          /* restart after successfull match & replace */
    else
