 ...
 *
 *  Version: $Id: Sc3.c 2403 2003-11-04 17:29:10Z thiadmer $
 */
#include <assert.h>
#include <stdio.h>
...
SC_FUNC int hier14(value *lval1)
{
  ...
  if (lval3.ident==iARRAY || lval3.ident==iREFARRAY) {
    ...
    check_userop(NULL,lval2.tag,lval3.tag,2,&lval3,&lval3.tag);
    ...
    if (tok=='[' || tok=='{') { /* subscript */
      ...
      if (lval2.ident==iCONSTEXPR) {    /* constant expression */
        ...
        if (close==']') {
          ...
          /* character index */
          if (lval2.constval<0 || sym->dim.array.length!=0 && sym->dim.array.length*((8*sizeof(cell))/charbits)<=lval2.constval)
            error(32,sym->name);        /* array index out of bounds */
          if (lval2.constval!=0) {
            /* don't add offsets for zero subscripts */
            if (charbits==16)
