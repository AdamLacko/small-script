 ...
 *
 *  Copyright (c) ITB CompuPhase, 1998-2003
...
#include <assert.h>
  ...
  #include <conio.h>
    ...
    #endif
    #include <windows.h>
    #if !defined amx_Init && !defined NO_WIN32_CONSOLE
      #include "console.h"
...
#define CMDWINDOW       (SCREENLINES - MAXWATCHES - DEF_LIST - 2)
  ...
  #define DEF_TERMINAL  TERM_WIN32
#else
  #define CHR_PROMPT    '>'
  #define CHR_HLINE     '-'
  #define CHR_VLINE     '|'
...
static char chr_vline=CHR_VLINE;
  ...
  #define dgetch          getch
  #if defined READLINE
    #define dgetstr(s,n)  rl_gets(s,n)
  #else
...
{
  assert(terminal==TERM_ANSI || terminal==TERM_VT100 || terminal==TERM_WIN32);
  if (terminal==TERM_ANSI || terminal==TERM_VT100)
    dprintf("\033[K");
#if defined WIN32_CONSOLE
  else if (terminal==TERM_WIN32)
...
static void term_setattrib(int bold)
{
  if (terminal==TERM_ANSI || terminal==TERM_VT100) {
    ...
    if (bold)
      win32_textcolor(15,0);    /* set "bold" */
    else
      win32_textcolor(7,0);     /* reset "bold" */
#endif
  } /* if */
}
...
static void term_csrset(int x,int y)
{
  ...
  if (terminal==TERM_ANSI || terminal==TERM_VT100)
    ...
    win32_gotoxy(x-1,y-1);  /* Win32 cursor starts (0,0) */
#endif
}
...
static void term_csrsave(void)
{
  ...
  if (terminal==TERM_ANSI) {
    dprintf("\033[s");
  } else if (terminal==TERM_VT100) {
    dprintf("\0337");
#if defined WIN32_CONSOLE
  } else if (terminal==TERM_WIN32) {
    win32_wherexy(&csrsave_x,&csrsave_y);
#endif
  } /* if */
}
...
static void term_csrrestore(void)
{
  ...
  if (terminal==TERM_ANSI) {
    ...
    win32_gotoxy(csrsave_x,csrsave_y);
#endif
  } /* if */
}
...
static void source_list(int startline, int numlines)
{
  ...
  if (terminal==TERM_ANSI || terminal==TERM_VT100 || terminal==TERM_WIN32) {
    term_csrsave();
...
static void printvalue(long value, int disptype)
{
  if (disptype==DISP_FLOAT)
    dprintf("%f", amx_ctof(value));
  else if (disptype==DISP_FIXED)
    dprintf("%ld.%03ld", value/1000, (value%1000+1000)%1000);
...
static void display_variable(AMX *amx,SYMBOL *sym,int index,int idxlevel)
{
  if ((sym->type==iARRAY || sym->type==iREFARRAY) && idxlevel==0) {
    if (sym->disptype==DISP_STRING) {
      ...
      dprintf("(multi-dimensional array)");
    } /* if */
  } else if ((sym->type==iARRAY || sym->type==iREFARRAY) && sym->length[0]>0 && index>=sym->length[0]) {
    dprintf("(index out of range, size=%d)",sym->length[0]);
...
static int break_check(AMX *amx,int line,int file)
{
  ...
  for (index=0; index<MAXBREAKS; index++) {
    if (breakpoints[index].type==BP_CODE && (int)breakpoints[index].addr==line
...
static void listcommands(char *command)
{
             ...
             || stricmp(command,"s")==0 || stricmp(command,"step")==0)
  {
    ...
    dprintf("\tTERM ANSI\tuse ANSI terminal display\n"
            ...
            "\tTERM WIN32\tuse Windows console interface\n"
  ...
  for ( ;; ) {
    ...
    if (stricmp(command,"?")==0) {
      ...
      if (stricmp(params,"off")==0)
        ...
        new_term=TERM_VT100;
#if defined WIN32_CONSOLE
      else if (stricmp(params,"win32")==0)
        new_term=TERM_WIN32;
#endif
      else
      ...
      if (*params=='*') {
        ...
        dprintf("\tInvalid command syntax, use \"? cw\" for details\n");
      } /* if */
      watch_list(amx,calllevel);
    } else {
